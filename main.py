...
# otp_store = {}  # deprecated - using Redis  # TEMP STORAGE FOR OTP
...
#     await app.state.redis.set(f"otp:{data.email}", str({), ex=300)
#         "otp": otp,
...
#     saved = await app.state.redis.get(f"otp:{data.email}")
...
#     await app.state.redis.delete(f"otp:{data.email}")
...
try:
    ...
    # fallback for older environments (pydantic v1)
...
# --- Redis integration added by assistant ---
import redis.asyncio as aioredis
import json
# use pydantic-settings BaseSettings (pydantic v2)
from pydantic_settings import BaseSettings
...
class Settings(BaseSettings):
    ...
    model_config = {
        ...
        "extra": "allow",   # <<-- allow extra keys like MONGO_URI
    }
